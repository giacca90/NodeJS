...
describe('Testing Unitarios', function() {
    ...
    afterEach(function(done) {
        Reserva.deleteMany({}).then((err, success) => {
            if(err) console.log(err);
            Usuario.deleteMany({}).then((err, success) => {
                if(err) console.log(err);
                Bicicleta.deleteMany({}).then((err, success) => {
                    if(err) console.log(err);
                    done();
        ...
        it('debe existir la reserva', (done) => {
            ...
            usuario.save().then(() => {
                ...
                bicicleta.save().then(async () => {
                    ...
                    var res = Reserva.find({}).populate('bicicleta').populate('usuario').exec();
